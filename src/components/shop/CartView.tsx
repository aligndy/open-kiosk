  ...
  return (
    <>
      ...
      <header className="sticky top-0 z-10 flex items-center bg-white px-4 py-3 shadow-sm">
        ...
        <h1 className="flex-1 text-center text-2xl font-bold text-gray-900">{t("cart.title")}</h1>
        <div className="w-12" />
      </header>
      ...
      {items.length === 0 ? (
        <div className="flex flex-1 flex-col items-center justify-center gap-6 px-4">
          ...
          <p className="text-2xl font-bold text-gray-400">{t("cart.empty")}</p>
          <button
            onClick={onBack}
        ...
        <>
                ...
                <div className="flex gap-3">
                  ...
                  <div className="h-20 w-20 shrink-0 overflow-hidden rounded-lg bg-gray-100">
                    {item.imageUrl ? (
                      <img
                        src={item.imageUrl}
                        alt={getTranslation(item.menuNameTranslations, currentLanguage, item.menuName)}
                  ...
                  <div className="flex flex-1 flex-col">
                    <div className="flex items-start justify-between">
                      <h3 className="text-xl font-bold text-gray-900">
                        {getTranslation(item.menuNameTranslations, currentLanguage, item.menuName)}
                      </h3>
                      <button
                    ...
                    {item.selectedOptions.length > 0 && (
                      <div className="mt-1 text-base text-gray-500">
                        {item.selectedOptions.map((opt, i) => (
                          <span key={i}>
                            {i > 0 && " / "}
                            {getTranslation(opt.optionNameTranslations, currentLanguage, opt.optionName)}
                            {opt.priceModifier !== 0 && (
                              <span className="text-gray-400">
                                ({opt.priceModifier > 0 ? "+" : ""}{fp(opt.priceModifier)})
                              </span>
                            )}
                          </span>
                        ))}
          ...
          <div className="border-t border-gray-200 bg-white px-4 py-4">
            <div className="mb-3 flex items-center justify-between">
              <span className="text-xl font-bold text-gray-700">{t("cart.totalAmount")}</span>
